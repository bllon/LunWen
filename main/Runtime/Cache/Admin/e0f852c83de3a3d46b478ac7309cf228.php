      ...
      <div class="main-panel">
        <div class="content-wrapper">
            ...
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php echo U('./borrow_list');?>">论文管理</a></li>
                <li class="breadcrumb-item active" aria-current="page">借阅列表</li>
              </ol>
            </nav>
          </div>
          <div class="row">
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <table class="table table-dark">
                    ...
                    <tbody>
	                    <?php if($borrowlist): if(is_array($borrowlist)): foreach($borrowlist as $key=>$borrow): ?><tr>
		                        <td>
		                          <?php echo ($borrow["borrow_id"]); ?>
		                        </td>
		                        <td>
		                      		<?php echo ($borrow["consumer_unionid"]); ?>
		                        </td>
		                        <td>
		                      		<?php echo ($borrow["lunwenid"]); ?>
		                        </td>
		                        <td>
		                      		<?php echo ($borrow["phone"]); ?>
		                        </td>
		                        <?php if($borrow["borrow_statu"] == 1): ?><td class="text-success">
		                        		待审核
		                        	</td>
		                        	<td>
			                      		<a class="btn btn-sm btn-danger" href="<?php echo U('./finish_borrow',array('borrow_id'=>$borrow['borrow_id']));?>">确定借阅</a>
			                        </td>
		                        <?php else: ?>
	                    ...
	                    <nav aria-label="Page navigation" style="margin-top:40px;">
						  <ul class="pagination pull-right">
						    <li>
						      <a class="btn btn-sm btn-dark" href="<?php echo U('./borrow_list',array('to'=>'prev','page'=>$current));?>" aria-label="Previous">
						        <span aria-hidden="true">&laquo;</span>
						      </a>
						    </li>
					    	<?php if(is_array($nav)): foreach($nav as $k=>$page): if($k== $current): ?><li><a  class="btn btn-sm btn-danger" href="<?php echo U('./borrow_list',array('page'=>$k));?>"><?php echo ($k); ?></a></li>
								<?php else: ?>
									<li><a  class="btn btn-sm btn-dark" href="<?php echo U('./borrow_list',array('page'=>$k));?>"><?php echo ($k); ?></a></li><?php endif; endforeach; endif; ?>
						    <li>
						      <a  class="btn btn-sm btn-dark" href="<?php echo U('./borrow_list',array('to'=>'next','page'=>$current));?>" aria-label="Next">
                  ...
                  <?php else: ?>
					<tr>
						<td class="text-center" colspan="8">
							<strong class="text-danger text-center">暂无论文借阅</strong>
						</td>
					</tr>
					</tbody>
	        		</table><?php endif; ?>
