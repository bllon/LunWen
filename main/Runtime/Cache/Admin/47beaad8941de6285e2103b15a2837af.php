        ...
        <div class="content-wrapper"> 
          ...
          <div class="page-header">
            ...
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="<?php echo U('./search');?>">论文管理</a></li>
                <li class="breadcrumb-item active" aria-current="page">论文搜索</li>
              </ol>
            </nav>
          </div>  
      	  <form action="<?php echo U('./search');?>" method="post">
	            ...
	            <div class="input-group">
	              <input type="text" name="keyWords" class="form-control" placeholder="关键字 / 论文标题" aria-label="Recipient's username" aria-describedby="basic-addon2">
      	  ...
      	  <?php if($lunwenlist): ?><p class="text text-dark bg-secondary" style="padding:20px 10px;letter-spacing:1px;">
      	  		关于词条&nbsp;<span class="text-info" style="font-size:18px;"><?php echo ($keywords); ?></span>
      	  		搜索到的结果有&nbsp;&nbsp;<span class="text-info"><?php echo ($listnum); ?>条</span>
      	  	</p><?php endif; ?>
      	  <div class="row">
      	  	<?php if(is_array($lunwenlist)): foreach($lunwenlist as $key=>$lunwen): ?><div class="col-12 grid-margin stretch-card">
	              <div class="card">
	                <div class="card-body">
	                  <h4 class="card-title"><?php echo ($lunwen["lunwen_title"]); ?></h4>
	                  <p class="card-description">
	                 		作者&nbsp;&nbsp;<span class="text-black"><?php echo ($lunwen["writer"]); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;<span><?php echo ($lunwen["addtime"]); ?></span>
	                  <ul class="list-inline">
	                  	<?php if(is_array($lunwen["thumb_img"])): foreach($lunwen["thumb_img"] as $key=>$img): ?><li class="list-inline-item">
		                  		<img src="http://xubeixyz123.com/LunWen/main/<?php echo ($img); ?>" style="width:100px;height:100px;padding:10px 20px;display:block;"/>
...
<script>
	$('.stretch-card').each(function(i){
		$('.stretch-card').eq(i).find('.list-inline-item').each(function(j){
			$('.stretch-card').eq(i).find('.list-inline-item').eq(j).find('p').html('第'+(j+1)+'页');
		});
	});
